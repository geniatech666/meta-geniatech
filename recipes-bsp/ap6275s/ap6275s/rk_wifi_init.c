...
static const char BOOT_MOD[]= "bootmod=";
...
static char wifi_moudule_path[200] = { 0 };
static char wifi_bootmod[64] = { 0 };
...
#define AP6255_BT_FIRMWARE_MODULE_PATH "/etc/firmware/BCM4345C0.hcd"
...
wifi_ko_file_name module_list[] =
{
	...
	{"APXXX",           BCM_DRIVER_MODULE_PATH, UNKKOWN_DRIVER_MODULE_ARG, AP6255_BT_FIRMWARE_MODULE_PATH},
...
int wifibt_load_driver(void)
{
	...
	int i = 0;
	int count = 10;//100; /* wait at most 20 seconds for completion */
	char temp[500] = {0};
	...
	/* check the wifi moduke ,ko file  form the params*/
	if (strncmp(wifi_bootmod, BOOT_MOD, 8) == 0){
		wifi_ko_path = wifi_moudule_path;
		wifi_ko_arg = wifi_bootmod;
	}
...
int main(int argc, char *argv[])
{
	...
	printf("Rockchip Linux WifiBt init,argc:%d\n",argc);

	for (i=1;i<argc;i++ ){

		switch (i) {
			case 1: 
                                strcpy(bt_tty_dev, argv[1]);
                                printf("BT TTY: %s \n", bt_tty_dev);
                                break;

			case 2:
				strcpy(wifi_moudule_path, argv[2]);
				printf("wifi_moudule_path: %s \n", wifi_moudule_path);
				break;

			case 3:
				strcpy(wifi_bootmod, argv[3]);
				printf("wifi_bootmod: %s \n", wifi_bootmod);
				break;
			default:
				printf("the params null\n");
		}
